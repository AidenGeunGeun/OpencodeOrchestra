...
export const ReadTool = Tool.define("read", {
  ...
  parameters: z.object({
    ...
    offset: z.coerce.number().int().describe("The line number to start reading from (1-indexed)").optional(),
    limit: z.coerce.number().int().describe("The maximum number of lines to read (defaults to 2000)").optional(),
  }),
  async execute(params, ctx) {
    ...
    const content = raw.map((line, index) => {
      return `${(index + offset).toString().padStart(5, "0")}| ${line}`
    })
