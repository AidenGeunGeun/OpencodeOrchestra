...
export class AccountRepo extends ServiceMap.Service<AccountRepo, AccountRepo.Service>()("@oco/AccountRepo") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      const query = <A>(f: (db: DbClient) => A) =>
        Effect.try({
          try: () => Database.use(f),
          catch: (cause) => new AccountRepoError("Database operation failed", { cause }),
        })
      ...
      const current = (db: DbClient) => {
        ...
        if (!account) return
        return Account.parse({ ...account, active_org_id: state.active_org_id ?? null })
      }

      const persistState = (db: DbClient, accountID: AccountID, orgID: Option.Option<OrgID>) =>
        db
          .insert(AccountStateTable)
          .values({
            id: ACCOUNT_STATE_ID,
            active_account_id: accountID,
            active_org_id: Option.getOrNull(orgID),
          })
          .onConflictDoUpdate({
      ...
      const active = Effect.fn("AccountRepo.active")(() =>
        query((db) => current(db)).pipe(Effect.map((row) => (row ? Option.some(row) : Option.none()))),
      )
      ...
      const list = Effect.fn("AccountRepo.list")(() =>
        query((db) =>
          db
            ...
            .from(AccountTable)
            .all()
            .map((row: AccountRow) => Account.parse({ ...row, active_org_id: null })),
        ),
      )
      ...
      const use = Effect.fn("AccountRepo.use")((accountID: AccountID, orgID: Option.Option<OrgID>) =>
        query((db) => persistState(db, accountID, orgID)).pipe(Effect.asVoid),
      )
      ...
      const getRow = Effect.fn("AccountRepo.getRow")((accountID: AccountID) =>
        query((db) => db.select().from(AccountTable).where(eq(AccountTable.id, accountID)).get()).pipe(
          Effect.map((row) => (row ? Option.some(row) : Option.none())),
        ),
      )
      ...
      const persistAccount = Effect.fn("AccountRepo.persistAccount")((input) =>
        tx((db) => {
          db.insert(AccountTable)
            ...
            .run()
          void persistState(db, input.id, input.orgID)
        }).pipe(Effect.asVoid),
      )
