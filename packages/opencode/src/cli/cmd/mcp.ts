    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in oco.jsonc:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .oco/ subdirectory too)
  const candidates = [
    path.join(baseDir, Global.Namespace.configFilenames[0]),
    path.join(baseDir, Global.Namespace.configFilenames[1]),
  ]

  if (!global) {
    candidates.push(
      path.join(baseDir, Global.Namespace.projectDir, Global.Namespace.configFilenames[0]),
      path.join(baseDir, Global.Namespace.projectDir, Global.Namespace.configFilenames[1]),
    )
  }
  ...
  // Default to oco.jsonc if none exist
  return candidates[0]
}
