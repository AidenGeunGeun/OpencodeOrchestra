...
const hasAssistantStructured = "structured" in ((MessageV2Any.Assistant as any)?.shape ?? {})
...
describe("StructuredOutput Integration", () => {
  test.skipIf(!hasApiKey || !hasAssistantStructured)(
    ...
    async () => {
      await withInstance(async () => {
        ...
        if (result.info.role === "assistant") {
          ...
          const output = structured as any
  ...
  test.skipIf(!hasApiKey || !hasAssistantStructured)(
    ...
    async () => {
      await withInstance(async () => {
        ...
        expect(result.info.role).toBe("assistant")
        if (result.info.role === "assistant") {
          const output = (result.info as any).structured as any
  ...
  test.skipIf(!hasApiKey)(
    ...
    async () => {
      await withInstance(async () => {
        ...
        expect(result.info.role).toBe("assistant")
        if (result.info.role === "assistant") {
          expect((result.info as any).structured).toBeUndefined()
          expect(result.info.error).toBeUndefined()
        }
  ...
  test.skipIf(!hasStructuredOutputError)("unit test: StructuredOutputError is properly structured", () => {
