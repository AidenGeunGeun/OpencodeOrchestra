...
const MessageV2Any = MessageV2 as any
const SessionPromptAny = SessionPrompt as any
const hasStructuredOutputError = typeof MessageV2Any.StructuredOutputError === "function"
const hasAssistantStructured = "structured" in (MessageV2Any.Assistant?.shape ?? {})
const hasCreateStructuredOutputTool = typeof SessionPromptAny.createStructuredOutputTool === "function"
...
describe.skipIf(!hasStructuredOutputError)("structured-output.StructuredOutputError", () => {
  test("creates error with message and retries", () => {
    const error = new MessageV2Any.StructuredOutputError({
  ...
  test("isInstance correctly identifies error", () => {
    ...
    expect(MessageV2Any.StructuredOutputError.isInstance(error)).toBe(true)
    expect(MessageV2Any.StructuredOutputError.isInstance({ name: "other" })).toBe(false)
  })
})
...
describe.skipIf(!hasAssistantStructured)("structured-output.AssistantMessage", () => {
  ...
  test("assistant message accepts structured", () => {
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      expect((result.data as any).structured).toEqual({ company: "Anthropic", founded: 2021 })
    }
  })
...
describe.skipIf(!hasCreateStructuredOutputTool)("structured-output.createStructuredOutputTool", () => {
  test("creates tool with correct id", () => {
    const tool = SessionPromptAny.createStructuredOutputTool({
      schema: { type: "object", properties: { name: { type: "string" } } },
      onSuccess: () => {},
    })
